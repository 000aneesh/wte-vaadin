package com.app.wte.constants;

public class WTEConstants {

	// public static final String TXT_FILE_EXTN = ".txt";
	// public static final String UNDERSCORE = "_";
	// public static final String EXT_PROPERTY_FOLDER = "wte-properties";
	// public static final String UPLOAD_FOLDER = "wte-temp";
	// public static final String EXT_PROPERTY_FILE = "wte.properties";
	// public static final String TEST_INPUT_FILE = "test_input.txt";
	// public static final String EXCEUTION_STATUS_FILE = "execution-status.xml";
}
